...
class AbstractSlamBamIterator(ABC):
        ...
        if read.has_tag("MP"):
            ...
            for mismatch in ngmMismatches:
                ...
                if readQlty >= self._minQual:
                    refPos = int(refPos) - 1
                    if read.is_reverse:
                        isSnpPos = self._snps is not None and self._snps.isAGSnp(
                            read.reference_name, read.reference_start + int(refPos))
                        readPos = read.query_length - readPos - 1
                    else:
                        isSnpPos = self._snps is not None and self._snps.isTCSnp(
                            read.reference_name, read.reference_start + int(refPos))

                    refPos = read.reference_start + refPos
...
class SlamSeqInterval:
    ...
    def __init__(
            ...
            tcReadCount,
            multimapCount,
            conversionRateLower=-1.0,
            conversionRateUpper=-1.0
    ):
    ...
    def __repr__(self):
        toString = "\t".join([
            ...
            self._strand,
            self._conversionRate,
            self._readsCPM,
            self._Tcontent,
            self._coverageOnTs,
            self._conversionsOnTs,
            self._readCount,
            self._tcReadCount,
            self._multimapCount,
            self._conversionRateLower,
            self._conversionRateUpper
        ])
        return toString
...
class SlamSeqAlignmentPosition:
    def __init__(
            ...
            readBaseQlty,
            isSnpPos,
            referenceBase5PrimeContext,
            referenceBase3PrimeContext
    ):
        # Base in read
    ...
    def __repr__(self):
        toString = ",".join([
            ...
            self.referenceBase,
            self.referenceBase5PrimeContext,
            self.referenceBase3PrimeContext,
            self.readPosition,
            self.readBase,
            self.readBaseQlty,
            self.isSnpPosition
    ...
    def __eq__(self, other):
        return (isinstance(other, SlamSeqAlignmentPosition)
                and self.readBase == other.readBase
                and self.referenceBase == other.referenceBase
                and self.readBaseQlty == other.readBaseQlty
                and self.readPosition == other.readPosition
                and self.referencePosition == other.referencePosition
...
class SlamSeqRead:
    def __init__(
        ...
        self.startRefPos = startRefPos
        # End position in the reference
        self.endRefPos = endRefPos
        # Chr
        self.chromosome = chromosome
        # Multiple TC-conversion flag
        self.isTcRead = isTcRead
        # Read is Multimapper
        self.isMultimapper = isMultimapper
        # List of mismatches in alignment
        self.isPaired = isPaired
        self.unmappedMate = unmappedMate
        self.duplicate = duplicate
        self.isSecondRead = isSecondRead
    ...
    def __repr__(self):
        return "\t".join([
            ...
            str(self.tCount),
            str(self.tcRate),
            self.conversionRates.__repr__(),
            str(self.startRefPos),
            str(self.endRefPos),
            self.mismatches.__repr__(),
            str(self.isTcRead),
            str(self.isMultimapper)]
        )
...
class SlamSeqBamIterator(AbstractSlamBamIterator):
...
class SlamSeqBamFile:
    ...
    def readInRegion(self, chromosome, start, stop, strand, maxReadLength, minQual=0, conversionThreshold=1):
        if self.isInReferenceFile(chromosome):
            ...
            if chromosome in self._bamFile.references:
                return SlamSeqBamIterator(
                    self._bamFile.fetch(reference=chromosome, start=max(0, start), end=min(chromosomeLength, stop)),
                    refSeq,
                    chromosome,
                    start,
                    strand,
    ...
    def readsInChromosome(self, chromosome, minQual = 0, conversionThreshold = 1):
        if chromosome in self._bamFile.references:
            ...
            return SlamSeqBamIterator(
                self._bamFile.fetch(reference=chromosome),
                refSeq,
                chromosome,
                1,
                ".",
...
class SlamSeqIter(AbstractSlamBamIterator):
    def __init__(
            self,
            bamFile,
            referenceFile,
        ...
        self._referenceFile = pysam.FastaFile(referenceFile)
        self._snps = snps
        self._iter = self._bamFile.__iter__()
        self._conversionThreshold = conversionThreshold
        self._minQual = minQual
    ...
    def __next__(self):
        # Create SlamSeqRead
        read = self._iter.__next__()
        slamSeqRead = SlamSeqRead(
            name=read.query_name,
            sequence=read.query_sequence,
            direction=ReadDirection.Reverse if read.is_reverse else ReadDirection.Forward,
            startRefPos=read.reference_start if not read.is_reverse else read.reference_end,
            endRefPos=read.reference_end if not read.is_reverse else read.reference_start,
            chromosome=read.reference_name,
            isPaired=read.is_paired,
            unmappedMate=read.mate_is_unmapped,
            duplicate=read.is_duplicate,
            isSecondRead=read.is_read2,
        )
